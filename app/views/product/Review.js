  ...
  _onSubmit = (obj) => {
    ...
    Utils.ajx({
      ...
      uri:
        ...
        '&Filter=ProductId:' + integrationId +
        '&Action=submit' +
        '&ReviewText=' + data.comment +
        '&Title=' + data.title +
        '&Rating=' + data.points +
        '&User=' + userBazaarvoiceToken +
        '&agreedtotermsandconditions=' + data.agreement +
        '&isrecommended=' + data.recommends
    }, (result) => {
      if (result['type'] == 'success') {
        ...
        this.setState({ message: 'Geçici bir hata oluştu. Daha sonra tekrar dene.' })
      }
    });
  }
    ...
    if (success) {
      _content = (
        <View style={{ padding: 40 }}>
          <View style={{ marginBottom: 60, alignSelf: 'center', position: 'relative', width: 150, height: 150, }}>
            <Animated.Image source={ICONS['reviewSuccess']} style={{ position: 'absolute', width: 150, height: 150, marginTop: _top, transform: [{ rotate: _rotate }], resizeMode: 'contain', }} />
            <Animated.Image source={ICONS['reviewHeart']} style={{ position: 'absolute', width: 150, height: 150, opacity: _opacity, marginTop: _top2, resizeMode: 'contain', }} />
          </View>
          <Text style={{ fontSize: 16, textAlign: 'center', marginBottom: 70, }}>Yorumun başarı ile gönderildi.</Text>
          <DefaultButton name="GERİ DÖN" callback={this._onBackToDetailsPress} boxColor="#000000" textColor="#ffffff" borderColor="#ffffff" />
        </View>)
    } else {
      _content = <Form callback={this._onSubmit} data={FORMDATA['review_submission']} />;
    }
