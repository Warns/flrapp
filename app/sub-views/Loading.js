...
const TOP_MARGIN =
  Platform.OS === "ios"
    ? DIMENSIONS.height == 812 || DIMENSIONS.height == 896
      ? 30
      : 20
...
class Loading extends React.Component {
  ...
  _loadInitialState = async () => {
    globals.getSecureStorage("__OPTIN__", answer => {
      if (answer !== "no") {
        ...
        if (_optin.userLoggedOut == true) this._getOut(false);
        else {
          globals.fetch(
  ...
  _Continue = login => {
    ...
    } else this.props.navigation.navigate("Splash");
  };
  ...
  render() {
    ...
    return (
      ...
      >
        <Animated.View
          style={[
            { zIndex: 2, opacity: _opacity, transform: [{ scale: _image }] },
            _absolute
          ]}
        >
          <Image
            source={require("root/assets/images/logo-cp.png")}
        ...
        />
        <Image
          source={require("root/assets/gifs/three-dots.gif")}
          style={{
            resizeMode: "cover",
            width: 60,
            height: 60,
