...
class PhoneConfirmation extends React.Component {
    ...
    if (canClick) {
      ...
      if (time > 0) {
        if (obj.data.mobilePhoneConfirmation == phone_verification) {
          ...
          this.setState({
            message: "Onay kodu geçerli değil, Lütfen kontrol edip tekrar dene."
          });
        }
      }
    }
  ...
  _Continue = () => {
    this.props.dispatch({
      type: "UPDATE_OPTIN",
      value: { registered_email: this.state.email, phone_checked: true }
    });
        ...
        <KeyboardAvoidingView style={{ flex: 1 }}>
          <View style={{ padding: 40, paddingBottom: 10, paddingTop: 20 }}>
            <Text style={{ color: "#000000", lineHeight: 18, fontSize: 15 }}>
              Lütfen {phone} numarali cep telefonuna SMS ile gelen onay kodunu
              gir.
            </Text>
          ...
          <Form
            ...
            data={FORMDATA["optin_phoneConfirmation"]}
          />
        </KeyboardAvoidingView>
      </SafeAreaView>
    );
