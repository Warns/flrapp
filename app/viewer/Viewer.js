      ...
      ({ type }) => {
        if (type == "ok") {
          Globals.AJX(
            {
              _self: _self,
              uri: Utils.getURL({ key: "cart", subKey: "deleteCartLine" }),
...
class CustomDetailContentItem extends Component {
  ...
  CUSTOM_STYLE = {
    tagsStyles: {
      ...
      b: { fontSize: 16, color: "#000000", lineHeight: 24, fontFamily: "Bold" }
    },
  ...
  _getView = () => {
    ...
    return content.map((item, order) => {
      let { title = "", content = "<i></i>", img = null, video = "" } = item;
      if (img != "" && img != null)
        img = (
          <Image
      ...
      if (title != "")
        return (
          <Text
            style={{ fontFamily: "Bold", fontSize: 16, marginTop: 15 }}
            key={order}
          >
            {title}
          </Text>
        );
      else if (video != "") return null;
      else
        return (
...
class CustomDetailListItem extends Component {
  ...
  _getDsc = () => {
    ...
    if (shortDesc == "") shortDesc = desc.substr(0, 100);
  ...
  _changeProduct = id => {
    store.dispatch({
      type: SHOW_CUSTOM_POPUP,
      value: { visibility: false, data: {}, type: "", itemType: "" }
    });
    store.dispatch({
      type: OPEN_PRODUCT_DETAILS,
...
class FeedsItem extends Component {
  ...
  _getLike = () => {
    ...
    return (
      <RatingButton
        onPress={_self._onRatingClicked}
        id="rating"
        value={like}
        userLike={userLike}
  ...
  _getIcon = () => {
    ...
    if (type == FEEDSTYPE["VIDEO"]) source = ICONS["feedVideo"];
    else if (type == FEEDSTYPE["INSTAGRAM"]) source = ICONS["feedInstagram"];
    else if (type == FEEDSTYPE["CAMPAING"]) source = ICONS["feedCampaing"];
    ...
    return (
      <TouchableOpacity activeOpacity={1} onPress={_self._onPress}>
        <View style={{ borderWidth: 1, borderColor: "#dcdcdc" }}>
          <Image style={{ height: h }} source={{ uri: uri }} />
          {overlay}
          {_self._heartAnim()}
        </View>
      </TouchableOpacity>
  ...
  _getFeed = () => {
    ...
    if (FEEDSTYPE["PRODUCT"] == labels[0]) return _self._getProduct();
    else return _self._getImage();
  };
      ...
      view = (
        <View style={{ margin: 10, marginBottom: 20 }}>
          <View>
            ...
            />
            <Placeholder.Box height={300} width="100%" animate="fade" />
          </View>
          <View
            style={{
              height: 40,
          ...
          >
            ...
            >
              <Placeholder.Line animate="fade" height={15} width={117} />
            </View>
            <Image
              style={{
                width: 40,
