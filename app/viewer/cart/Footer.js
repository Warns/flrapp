...
import { ICONS, FORMDATA } from "root/app/helper/Constant";
  ...
  _getFormButton = () => {
    ...
    if (coupon && !cartNoResult)
      view = (
        ...
        >
          <Text
            style={{ fontFamily: "RegularTyp2", fontSize: 13, marginRight: 12 }}
          >
            {txt}
          </Text>
          <Image style={{ width: 9, height: 5 }} source={ICONS[ico]} />
        </TouchableOpacity>
      );
    return view;
  };
      ...
      prc = (
        <View style={{ alignItems: "flex-end" }}>
          <Text style={{ fontFamily: "Bold", fontSize: 15 }}>
            TOPLAM: {Utils.getPriceFormat(netTotal)}
          </Text>
          <View style={{ flexDirection: "row" }}>
            <Text style={{ fontFamily: "RegularTyp2", fontSize: 13 }}>
              İNDİRİM: {Utils.getPriceFormat(discountTotal)},{" "}
            </Text>
            <Text style={{ fontFamily: "RegularTyp2", fontSize: 13 }}>
              ARA TOPLAM: {Utils.getPriceFormat(subTotal)}
            </Text>
          </View>
        </View>
      );
