...
const DefaultButton = class DefaultButton extends React.Component {
  ...
  _onPress = () => {
    ...
    for (var i = 0; i < 10; ++i) {
      kisses.push(
        <Image
          key={"a" + i}
          source={ICONS["lips" + Math.floor(Math.random() * 4)]}
          style={{
            position: "absolute",
            left: SCREEN_DIMENSIONS.width * Math.random() * 0.8,
            top: SCREEN_DIMENSIONS.height * Math.random() * 0.8,
            transform: [
              { scale: Math.random() * 0.5 + 0.5 },
              { rotate: Math.random() * 45 - 45 + "deg" }
            ],
            width: 100,
            height: 50,
            resizeMode: "cover"
  ...
  render() {
    ...
    const textColorStyle = this.props.textColor
      ? this.props.textColor
      : "#000000";
    const borderColorStyle = this.props.borderColor
